    ...
    .then((res)=>{
      if(res.data.ok){
        ...
        if(res.data.person=="Student"){
         localStorage.setItem(`student-${res.data.id}`, res.data.token);
          navigate(`/student/profile/${res.data.id}`);
        }
          ...
          localStorage.setItem(`teacher-${res.data.id}`, res.data.token);
          navigate(`/teacher/profile/${res.data.id}`);
        }
      }
      else{
        alert("Password or Id does not match");
      }
    })
  }
