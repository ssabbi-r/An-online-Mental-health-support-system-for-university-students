...
const StudentProfile = () => {
  ...
  let token=localStorage.getItem(`student-${id}`);
  ...
  useEffect(()=>{
    if(!token){
      ...
      axios.get(`http://localhost:8000/student/info/${id}`,
        {
          headers:{
            Authorization:token
          }
  ...
  return <>
  {token && <div className={css.studentprofilepage}>
      ...
      <div className={css.navbar}>
        <div className={css.navbarstudentname}>{data.name}</div>
      </div>
