...
const TeacherProfile = () => {
  ...
  let token=localStorage.getItem(`teacher-${id}`);
  ...
  useEffect(()=>{
    if(!token){
      ...
      axios.get(`http://localhost:8000/teacher/info/${id}`,
        {
          headers:{
            Authorization:token
          }
 ...
 {token && <div className={css.teacherprofilepage}>
      ...
      <div className={css.body}>
        ...
        <div className={css.formcontainer}>
          <input type="text" placeholder="Choose an anonymous name" required />
